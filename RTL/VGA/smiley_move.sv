...
module	smiley_move	(	
					...
					input  logic collision_pacman_wall,         //collision if smiley hits an object
					input  logic collision_pacman_monster,         //collision if pacman hits an monster
					...
					output logic signed 	[10:0] Wall_topLeftX, // output the top left corner 
					output logic signed 	[10:0] Wall_topLeftY,// output the top left corner 
					output  logic [2:0] Wall_new_value
...
logic collision_pacman_wall_d;//to make a edge collision
...
//		toggle_x_key_D <= toggle_x_key ;  //shift register to detect edge 
		...
		collision_pacman_wall_d <= collision_pacman_wall;
...
is_block_per = (((offsetX_pacman%32) == 0) && ((offsetY_pacman%32) == 0));
		...
		if(collision_pacman_wall == 0) begin
       ...
       // collcting collisions 
		 //up
				if(MazeBitMapMask[(offsetY_pacman/32)-1][(offsetX_pacman/32)] == 4'h1 && pressed_numlock)begin
					Wall_topLeftX <= offsetX_pacman;
					Wall_topLeftY <= offsetY_pacman - 32;
					Wall_new_value <= 0;
					MazeBitMapMask[(offsetY_pacman/32)-1][(offsetX_pacman/32)] <= MazeBitMapMask[(offsetY_pacman/32)-1][(offsetX_pacman/32)] -1;
					end 
		//down
            if(MazeBitMapMask[(offsetY_pacman/32)+1][(offsetX_pacman/32)] == 4'h1 && pressed_numlock)begin
					Wall_topLeftX <= offsetX_pacman;
					Wall_topLeftY <= offsetY_pacman +32;
					Wall_new_value <= 0;
					MazeBitMapMask[(offsetY_pacman/32)+1][(offsetX_pacman/32)] <= 0;
					end 
		//left
				if(MazeBitMapMask[(offsetY_pacman/32)][(offsetX_pacman/32)-1] == 4'h1 && pressed_numlock)begin
					Wall_topLeftX <= offsetX_pacman -32;
					Wall_topLeftY <= offsetY_pacman;
					Wall_new_value <= 0;
					MazeBitMapMask[(offsetY_pacman/32)][(offsetX_pacman/32)-1] <= 0;
					end 
		//right
				if(MazeBitMapMask[(offsetY_pacman/32)][(offsetX_pacman/32)+1] == 4'h1 && pressed_numlock)begin
					Wall_topLeftX <= offsetX_pacman + 32;
				...
				if (collision_pacman_wall && !collision_pacman_wall_d) begin
		...
		//when collision_pacman_monster, pacman go to start point
